...
class StudentServiceTest extends BaseServiceTest {
    ...
    void testCalculateTotalCredits() throws Exception {
        ...
        int mathCourse = TestDataFactory.createTestCourse(
                getErpConnection(), mathDept, "MATH101", "Calculus", 3, null);
        int mathFaculty = TestDataFactory.createTestFaculty(
                getErpConnection(), 201, mathDept, "Professor", "Dr. Math");
        int mathSection = TestDataFactory.createTestSection(
                getErpConnection(), mathCourse, mathFaculty, "A", 50, "R102");
        TestDataFactory.createTestEnrollment(getErpConnection(), testStudentId, mathSection);
